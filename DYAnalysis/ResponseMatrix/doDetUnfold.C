...
void doDetUnfold( Bool_t isCorrected = kFALSE, TString Sample = "Powheg", TString HLTname = "PAL3Mu12", int run=0, var thevar = var::mass ) 
{
   ...
   // output
   TFile *fout = new TFile("ResponseMatrix/output.root","RECREATE");
   fout->cd();
   ...
   output.Write();
      ...
      histMunfold = unfold_MLE(h_Measured_TUnfold,h_response,histEmatTotal);
      histMdetFold = fold_MLE(histMunfold,h_response);
   }
