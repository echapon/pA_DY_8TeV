...
double HFweight::weight(double hiHF, HFweight::HFside side, bool isPV1) {
   ...
   else if (side==Ntracks) hist = isPV1 ? h_hiNtracks_PV1_ratio : h_hiNtracks_ratio;
   ...
   int ibin = min(hist->FindBin(hiHF),hist->GetNbinsX());

   return min(5.,hist->GetBinContent(ibin));
}
