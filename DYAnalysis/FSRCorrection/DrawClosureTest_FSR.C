...
void DrawClosureTest_FSR(TFile *f, var thevar);

void DrawClosureTest_FSR(const char* matrixfile="FSRCorrection/ROOTFile_FSRCorrections_DressedLepton_Powheg_0.root") {
   TFile *f = TFile::Open(matrixfile);
   for (int i=0; i<var::ALLvar; i++) {
      var thevar_i = static_cast<var>(i);
      DrawClosureTest_FSR(f, thevar_i);
   }
}
void DrawClosureTest_FSR(TFile *f, var thevar) {
	setTDRStyle();
   ...
   double* binsv = binsvar(thevar);
	...
	// -- Response Matrix -- //
	///////////////////////////
	TH1D *h_Truth_TUnfold = (TH1D*)f->Get("h_" + var + "_preFSR");
   TH1D *h_Measured_TUnfold = (TH1D*) f->Get(Form("h_%s_postFSR",thevarname));
	TH2D* h_nEvents = (TH2D*)f->Get("h_" + var + "_postpreFSR_tot");
   ...
   TH1D *h_unfolded = unfold::unfold_MLE(h_Measured_TUnfold,h_RespM,histEmatTotal);
	...
	MyCanvas *myc = new MyCanvas(Form("FSRCorrection/c_ClosureTest_%s",thevarname), xaxistitle(thevar), "Number of events");
