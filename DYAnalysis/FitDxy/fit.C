...
void fit(const char* histfile, const char* outputfile) {
   ...
   for (int i=0; i<binnum; i++) {
      ...
      hmass->SetBinContent(i,((RooRealVar*) (result->floatParsFinal().find("ndy")))->getVal());
      hmass->SetBinError(i,((RooRealVar*) (result->floatParsFinal().find("ndy")))->getError());
      dir->cd();
      result->Write();
   }
   dir->cd();
...
RooFitResult* fit(const char* histfile, const char* varname, double varmin, double varmax) {
   ...
   // get histos
   TH1D *hdata = (TH1D*) gDirectory->Get("data_obs");;
   TH1D *hdy = (TH1D*) gDirectory->Get("DYMuMu");;
   TH1D *hdytautau = (TH1D*) gDirectory->Get("DYTauTau");;
   TH1D *htt = (TH1D*) gDirectory->Get("TT");;
   TH1D *hww = (TH1D*) gDirectory->Get("WW");;
   TH1D *hwz = (TH1D*) gDirectory->Get("WZ");;
   TH1D *hzz = (TH1D*) gDirectory->Get("ZZ");;
   TH1D *hdataSS1 = (TH1D*) gDirectory->Get("DataSS1");;
   TH1D *hdataSS2 = (TH1D*) gDirectory->Get("DataSS2");;
