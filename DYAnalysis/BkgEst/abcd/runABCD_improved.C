...
void runABCD_improved(TString trigger="PAL3Mu12", TString tag="DataMinusMC") { // tag can be DataMinusMC (nominal), QCD (for closure on MCs)
   ...
   for (int niso=0; niso<=3; niso++) {
      ...
      for (int i=0; i<5; i++) {
         ...
         TH1D *h_0iso_SS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_0iso_SS_Wjets");
         TH1D *h_0iso_OS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_0iso_OS_Wjets");
         TH1D *h_1iso_SS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_1iso_SS_Wjets");
         TH1D *h_1iso_OS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_1iso_OS_Wjets");
         TH1D *h_2iso_SS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_2iso_SS_Wjets");
         TH1D *h_2iso_OS_wjets = (TH1D*) tdir_inputs->Get("h_" + varnames[i] + "_2iso_OS_Wjets");
         ...
         for (int i=1; i<=houtput->GetNbinsX(); i++) {
            // find the k factor
            double k = (h_1iso_SS->GetBinContent(i)*h_0iso_OS->GetBinContent(i) - h_0iso_SS->GetBinContent(i)*h_1iso_OS->GetBinContent(i))
               / (h_0iso_OS->GetBinContent(i)*h_1iso_SS_wjets->GetBinContent(i) - h_0iso_SS->GetBinContent(i)*h_1iso_OS_wjets->GetBinContent(i));
            cout << k << endl;

            double A, B, C, D, dA, dB, dC, dD;
            if (niso==2) {
               A = h_2iso_SS->GetBinContent(i) - k*h_2iso_SS_wjets->GetBinContent(i);
               dA = h_2iso_SS->GetBinError(i);
               C = h_1iso_SS->GetBinContent(i) + h_0iso_SS->GetBinContent(i) - k*(h_1iso_SS_wjets->GetBinContent(i));
               dC = sqrt(pow(h_1iso_SS->GetBinError(i),2) + pow(h_0iso_SS->GetBinError(i),2));
               D = h_1iso_OS->GetBinContent(i) + h_0iso_OS->GetBinContent(i) - k*(h_1iso_OS_wjets->GetBinContent(i));
               dD = sqrt(pow(h_1iso_OS->GetBinError(i),2) + pow(h_0iso_OS->GetBinError(i),2));
            } else if (niso==1) {
               A = h_2iso_SS->GetBinContent(i) - k*h_2iso_SS_wjets->GetBinContent(i);
               dA = h_2iso_SS->GetBinError(i);
               C = h_1iso_SS->GetBinContent(i) - k*h_1iso_SS_wjets->GetBinContent(i);
               dC = h_1iso_SS->GetBinError(i);
               D = h_1iso_OS->GetBinContent(i) - k*h_1iso_OS_wjets->GetBinContent(i);
               ...
               A = h_1iso_SS->GetBinContent(i) - k*h_1iso_SS_wjets->GetBinContent(i);
            ...
            B = (C>0) ? (A*D)/C : 0;
            if (niso<3) B += k*h_2iso_OS_wjets->GetBinContent(i);
            else B += k*h_1iso_OS_wjets->GetBinContent(i);
