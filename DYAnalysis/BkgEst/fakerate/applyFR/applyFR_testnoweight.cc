// This is the modified applyFR.cc code to test the effect of weight with FR
...
void applyFR_testnoweight(SampleTag index) {
	...
	TFile* f = new TFile("histograms/fake"+TString(Name(index))+"_noFRweight.root","RECREATE");

	// histogram definition : X1 : weight with fake rate from template fitting, X2 : no weight
	// From the test, basically weight in every event is 1 without exception
	...
	for(int i=0; i!=nEntries; i++) {
		...
		if( failingMuons->size()>1 ) {
			if( dijetDY(event->dimuons,failingMuons,tempMuons) ) {
				...
				//std::cout << "dijet, FR1_template : " << FR1_template << std::endl;
				//std::cout << "dijet, weight_template = " << weight_template << std::endl;
				/*
			...
			if( wjetsDY(event->dimuons, failingMuons->at(0), passingMuons->at(0), tempMuons) ) {
				...
				//		std::cout << "wjet, FR1_template : " << FR1_template << std::endl;
				//		std::cout << "wjet, weight_template = " << weight_template << std::endl;
...
// produce all plots
void applyFR_testnoweight() {
	   for (int i=0; i<=QCD; i++) {
	//for (int i=Data1; i<=QCD; i++) { 
		SampleTag tag = static_cast<SampleTag>(i);
		if (tag==ALL) continue;
		cout << Name(tag) << endl;
		applyFR_testnoweight(tag);
	}
}
