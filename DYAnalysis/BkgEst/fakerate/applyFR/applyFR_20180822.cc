...
void applyFR_20180822(SampleTag index, TString infrname, TString rmk) {
    ...
    TFile* f = new TFile("histograms/fake_"+rmk+"_"+TString(Name(index))+".root","RECREATE");
...
// produce all plots
void applyFR_20180822(TString infrname, TString rmk) {
   for (int i=0; i<=QCD; i++) {
//	  for (int i=DYMuMu1030; i<=QCD; i++) {
//	  for (int i=0; i<DYMuMu1030; i++) {
//	  for (int i=Data2; i<=Data2; i++) {
      SampleTag tag = static_cast<SampleTag>(i);
      if (tag==ALL) continue;
      cout << Name(tag) << endl;
      applyFR_20180822(tag, infrname, rmk);
   }
}
