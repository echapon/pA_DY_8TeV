...
void estimateDijet_opt(TString rmk) {
...
//    f[QCD] = new TFile("histograms/fakeQCD_noFRweight.root");
	   for (int i=0; i<ALL; i++) f[i] = new TFile(Form("histograms/fake_%s_%s.root",rmk.Data(),(TString*)(Name(static_cast<SampleTag>(i)))));
      f[QCD] = new TFile(Form("histograms/fake_%s_QCD.root",rmk.Data()));
...
//###        double systematic = fabs(dijet->GetBinContent(i) - dijet_ratio[Data1]->GetBinContent(i));
    ...
    //TFile* gg = new TFile("result/dijet_rap1560_noFRweight.root","RECREATE");
	 TFile* gg = new TFile(Form("result/dijet_%s_%s.root",outtitle.Data(),rmk.Data()),"RECREATE"); 
    dijet->Write();
