   ...
   CentralityBinTag_(consumes<int>(iConfig.getParameter<edm::InputTag>("CentralityBinSrc")))
{
	...
	theStoreCentralityFlag            = iConfig.getUntrackedParameter<bool>("StoreCentralityFlag", false);
...
void DYntupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   hiZDCminus=-1;
	...
	for( int i = 0; i < MPSIZE; i++ )
	{
		...
		Electron_passVetoID[i] = -999;
		Electron_passLooseID[i] = -999;
		Electron_passMediumID[i] = -999;
		Electron_passTightID[i] = -999;
   ...
   // }
	...
	// cout << "##### Analyze:PrimaryVertex Handle #####" << endl;

   if (theStoreCentralityFlag) {

      edm::Handle<int> cbin_;
      // not yet working (May 2017)!
	...
	for( unsigned i = 0; i != muonHandle->size(); i++ )
	{
		...
		if( imuon.isLooseMuon() ) 	isLooseMuon[_nMuon] = 1;
		if( imuon.isMediumMuon() ) 	isMediumMuon[_nMuon] = 1;
		if( imuon.isTightMuon(vtx) ) 	isTightMuon[_nMuon] = 1;
		if( imuon.isSoftMuon(vtx) ) 	isSoftMuon[_nMuon] = 1;
		if( imuon.isHighPtMuon(vtx) ) 	isHighPtMuon[_nMuon] = 1;
	...
	// cout << "##### Analyze:End of Muon Parts #####" << endl;
...
void DYntupleMaker::beginJob()
{
   ...
   if(theStoreCentralityFlag) {
      // Centrality
      DYTree->Branch("hiBin",&hiBin,"hiBin/I");
      DYTree->Branch("hiHF",&hiHF,"hiHF/F");
      DYTree->Branch("hiHFplus",&hiHFplus,"hiHFplus/F");
      DYTree->Branch("hiHFminus",&hiHFminus,"hiHFminus/F");
      DYTree->Branch("hiHFplusEta4",&hiHFplusEta4,"hiHFplusEta4/F");
      DYTree->Branch("hiHFminusEta4",&hiHFminusEta4,"hiHFminusEta4/F");

      DYTree->Branch("hiZDC",&hiZDC,"hiZDC/F");
      DYTree->Branch("hiZDCplus",&hiZDCplus,"hiZDCplus/F");
      DYTree->Branch("hiZDCminus",&hiZDCminus,"hiZDCminus/F");

      DYTree->Branch("hiHFhit",&hiHFhit,"hiHFhit/F");
      DYTree->Branch("hiHFhitPlus",&hiHFhitPlus,"hiHFhitPlus/F");
      DYTree->Branch("hiHFhitMinus",&hiHFhitMinus,"hiHFhitMinus/F");

      DYTree->Branch("hiET",&hiET,"hiET/F");
      DYTree->Branch("hiEE",&hiEE,"hiEE/F");
      DYTree->Branch("hiEB",&hiEB,"hiEB/F");
      DYTree->Branch("hiEEplus",&hiEEplus,"hiEEplus/F");
      DYTree->Branch("hiEEminus",&hiEEminus,"hiEEminus/F");
      DYTree->Branch("hiNpix",&hiNpix,"hiNpix/I");
      DYTree->Branch("hiNpixelTracks",&hiNpixelTracks,"hiNpixelTracks/I");
      DYTree->Branch("hiNtracks",&hiNtracks,"hiNtracks/I");
      DYTree->Branch("hiNtracksPtCut",&hiNtracksPtCut,"hiNtracksPtCut/I");
      DYTree->Branch("hiNtracksEtaCut",&hiNtracksEtaCut,"hiNtracksEtaCut/I");
      DYTree->Branch("hiNtracksEtaPtCut",&hiNtracksEtaPtCut,"hiNtracksEtaPtCut/I");
   }
	...
	DYTree->Branch("Electron_passVetoID", &Electron_passVetoID, "Electron_passVetoID[Nelectrons]/I");
	DYTree->Branch("Electron_passLooseID", &Electron_passLooseID, "Electron_passLooseID[Nelectrons]/I");
	DYTree->Branch("Electron_passMediumID", &Electron_passMediumID, "Electron_passMediumID[Nelectrons]/I");
	DYTree->Branch("Electron_passTightID", &Electron_passTightID, "Electron_passTightID[Nelectrons]/I");
	...
	DYTree->Branch("isLooseMuon", &isLooseMuon, "isLooseMuon[nMuon]/I");
	DYTree->Branch("isMediumMuon", &isMediumMuon, "isMediumMuon[nMuon]/I");
	DYTree->Branch("isTightMuon", &isTightMuon, "isTightMuon[nMuon]/I");
	DYTree->Branch("isSoftMuon", &isSoftMuon, "isSoftMuon[nMuon]/I");
	DYTree->Branch("isHighPtMuon", &isHighPtMuon, "isHighPtMuon[nMuon]/I");
...
void DYntupleMaker::hltReport(const edm::Event &iEvent)
{
	...
	if( !trigResult.failedToGet() )
	{
		...
		} // -- end of for( int itrig = 0; itrig != ntrigs; ++itrig) -- //
...
void DYntupleMaker::fillElectrons(const edm::Event &iEvent)
{
	...
	for(int i=0; i< (int)ElecHandle->size(); i++)
	{
		...
		Electron_passVetoID[_nElectron] = (int)isPassVeto;
		Electron_passLooseID[_nElectron] = (int)isPassLoose;
		Electron_passMediumID[_nElectron] = (int)isPassMedium;
		Electron_passTightID[_nElectron] = (int)isPassTight;
